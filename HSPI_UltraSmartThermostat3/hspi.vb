...
Public Class HSPI
  ...
  '
  Public ThermostatDataThread As Thread
...
#Region "HSPI - Base Plugin API"
      ...
      Try
        ...
        Select Case dv_type
          ...
          Case "Netatmo Setpoint"
            Dim dv_value As Double = Double.Parse(CC.ControlValue)

            Dim pdata As clsPlugExtraData = dv.PlugExtraData_Get(Nothing)
            If pdata IsNot Nothing Then
              Dim device_id As String = pdata.GetNamed("device_id")
              Dim module_id As String = pdata.GetNamed("module_id")

              Dim SetthermpointResponse As hspi_netatmo_api.SetthermpointResponse = NetatmoAPI.Setthermpoint(device_id, module_id, "manual", -1, dv_value)

              If SetthermpointResponse.status.ToLower = "ok" Then
                hs.SetDeviceValueByRef(CC.Ref, dv_value, True)
                If ThermostatDataThread.ThreadState = ThreadState.WaitSleepJoin Then
                  ThermostatDataThread.Interrupt()
                End If
              End If
          ...
          Case "Netatmo Setpoint Mode"
            ...
            If pdata IsNot Nothing Then
              ...
              '
              ' Get the current thermostat setpoint
              '
              Dim dv_addr2 As String = String.Format("{0}-Setpoint", module_id)
              Dim dev_ref As Integer = hs.DeviceExistsAddress(dv_addr2, False)
              Dim dv2 As Scheduler.Classes.DeviceClass = hs.GetDeviceByRef(dev_ref)

              Dim setpoint_mode As String = String.Empty
              Dim setpoint_temp As Double = dv2.devValue(Nothing)
              Select Case dv_value
                Case 0
                  setpoint_mode = "off"
                Case 1
                ...
                Case 5
                  setpoint_mode = "max"
                Case Else
                  Throw New Exception("Unable to set Thermostat Setpoint Mode because invalid options were specified.")
              End Select

              Dim SetthermpointResponse As hspi_netatmo_api.SetthermpointResponse = NetatmoAPI.Setthermpoint(device_id, module_id, setpoint_mode, -1, setpoint_temp)
  ...
  Public Function InitIO(ByVal port As String) As String Implements HomeSeerAPI.IPlugInAPI.InitIO
    ...
    Try
      ...
      If bDBInitialized = True Then
        ...
        '
        ThermostatDataThread = New Thread(New ThreadStart(AddressOf CheckThermostatDataThread))
        ThermostatDataThread.Name = "ThermostatDataThread"
        ThermostatDataThread.Start()

        WriteMessage(String.Format("{0} Thread Started", ThermostatDataThread.Name), MessageType.Debug)
  ...
  Public Sub ShutdownIO() Implements HomeSeerAPI.IPlugInAPI.ShutdownIO
    ...
    Try
      ...
      If ThermostatDataThread.IsAlive = True Then
        ThermostatDataThread.Abort()
      End If
